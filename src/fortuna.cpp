...
void CForTunaPool::GetDenominationsToString(int nDenom, std::string& strDenom){
    ...
    //
    // bit 0 - 100TPM+1 ( bit on if present )
    // bit 1 - 10TPM+1
    // bit 2 - 1TPM+1
    // bit 3 - .1TPM+1
    // bit 3 - non-denom
...
int CForTunaPool::GetDenominationsByAmounts(std::vector<int64_t>& vecAmount){
    ...
    // Make outputs by lootpmcoin through denominations, from small to large
