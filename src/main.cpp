...
// Copyright (c) 2017-2018 The TpmCoin developers
...
int nCoinbaseMaturity = 5; //30 on Mainnet D e n a r i u s, 20 for testnet//
...
const string strMessageMagic = "TpmCoin Signed Message:\n";
...
int64_t GetProofOfWorkReward(int nHeight, int64_t nFees)
{
	...
	if (pindexBest->nHeight == 1)
		...
		nSubsidy = 0.5 * COIN;
	else if (pindexBest->nHeight <= 3398) // V1.0.0 fork in block 3398/ Block 3398 ~ 6,796k TPM
		nSubsidy = 0.5 * COIN;
	else if (pindexBest->nHeight <= LAST_POW_BLOCK) // V1.0.0 / Block 1m ~ 993,204k TPM
		nSubsidy = 0.5 * COIN;	
	//else if (pindexBest->nHeight <= 1500000) // V1.0.0 / Block 2m ~1m TPM
	//	nSubsidy = 0.05 * COIN;
	//else if (pindexBest->nHeight <= 2000000) // V1.0.0 / Block 3m ~1m TPM
	//	nSubsidy = 0.08 * COIN;	
    else if (pindexBest->nHeight > LAST_POW_BLOCK) // Block 1.5m
		nSubsidy = 0; // PoW Ends ~ 4,500,000 Total TPM Mined via PoW
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    */
    ...
    // ----------- masternode payments -----------
    // Once upon a time, People were really interested in TPM.
    // So much so, People wanted to bring TPM to the moon. Even Mars, Sooner than the roadster...
    ...
    // People wanted masternodes... Really Bad. But King Carsen was already busy with the rest of TPM
        ...
        if(IsProofOfStake() && pindex != NULL){
            if(pindex->GetBlockHash() == hashPrevBlock){
                ...
                if (pindex != NULL)
                {
                        ...
                        if(vtx[1].vout[i].scriptPubKey == payee )
                            ...
                            BOOST_FOREACH(CMasterNode& mn, vecMasternodes)
                            {
                                ...
                                if (vtx[1].vout[i].scriptPubKey == pubScript)
                                {
                                    ...
                                    if (fDebug) printf("Masternode PoS payee found at block %d: %s who got paid %s TPM (last payment was %d blocks ago at %d)\n", pindex->nHeight+1, address2.ToString().c_str(), FormatMoney(vtx[1].vout[i].nValue / COIN).c_str(), paidAge, mn.nBlockLastPaid);
                    ...
                    if(fDebug) { printf("CheckBlock-POS() : Is initial download, skiptpmcoin masternode payment check %ld\n", pindexBest->nHeight+1); }
                }
            } else {
                if(fDebug) { printf("CheckBlock-POS() : Skiptpmcoin masternode payment check - nHeight %ld Hash %s\n", pindexBest->nHeight+1, GetHash().ToString().c_str()); }
            }
            ...
            if(pindex->GetBlockHash() == hashPrevBlock){
                ...
                if (pindex != NULL)
                {
                    ...
                    for (unsigned int i = 0; i < vtx[0].vout.size(); i++) {
                        ...
                        BOOST_FOREACH(CMasterNode& mn, vecMasternodes)
                        {
                            ...
                            if (vtx[0].vout[i].scriptPubKey == pubScript)
                            {
                                int lastPaid = mn.nBlockLastPaid;
                                int paidAge = pindex->nHeight+1 - lastPaid;
                                if (fDebug) printf("Masternode PoW payee found at block %d: %s who got paid %s TPM (last payment was %d blocks ago at %d)\n", pindex->nHeight+1, address2.ToString().c_str(), FormatMoney(vtx[0].vout[i].nValue).c_str(), paidAge, mn.nBlockLastPaid);
                                if (paidAge < 150) // TODO: Probably make this check the MN is in the top 50?
                                {
                                    if (fDebug) printf("WARNING: This masternode payment is too aggressive and will not be accepted in v3+\n");
                                }
                    ...
                    if(fDebug) { printf("CheckBlock-POW() : Is initial download, skiptpmcoin masternode payment check %ld\n", pindexBest->nHeight+1); }
                }
            } else {
                if(fDebug) { printf("CheckBlock-POW() : Skiptpmcoin masternode payment check - nHeight %ld Hash %s\n", pindexBest->nHeight+1, GetHash().ToString().c_str()); }
            }
        }

         else {
            if(fDebug) { printf("CheckBlock() : pindex is null, skiptpmcoin masternode payment check\n"); }
        }
    } else {
        if(fDebug) { printf("CheckBlock() : skiptpmcoin masternode payment checks\n"); }
    }
...
bool ProcessBlock(CNode* pfrom, CBlock* pblock)
{
    ...
    // TpmCoin: ask for pending sync-checkpoint if any
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        strMiscWarning = strMessage;
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "TpmCoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (fTestNet)
    {
        pchMessageStart[0] = 0xb7;//b713e506
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "https://www.coindesk.com/csbs-firms-explained";
        CTransaction txNew;
        txNew.nTime = 1600624849;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nTime    = 1600624849;
        block.nVersion = 1;
        block.nBits    = bnProofOfWorkLimit.GetCompact();
		    block.nNonce   = 2556904;
        ...
        //// debug print
        assert(block.hashMerkleRoot == uint256("0xd765b6ac1ddec91167f2360db2fca9726e19fb902bf8258ff143d8d4cf772813"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0x31, 0x6d, 0x50, 0x74 };
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    {
        printf("dropmessagestest DROPTPM RECV MESSAGE\n");
        return true;
    }
    ...
    else if (strCommand == "getblocks")
    {
        ...
        for (; pindex; pindex = pindex->pnext)
        {
            ...
            {
                if (fDebugNet) printf("  getblocks stoptpmcoin at %d %s\n", pindex->nHeight, pindex->GetBlockHash().ToString().substr(0,20).c_str());
            ...
            if (--nLimit <= 0)
            {
                ...
                if (fDebugNet) printf("  getblocks stoptpmcoin at limit %d %s\n", pindex->nHeight, pindex->GetBlockHash().ToString().substr(0,20).c_str());
    ...
    else if (strCommand == "tpmcoin")
    {
        if (pfrom->nVersion > BIP0031_VERSION)
        {
            ...
            //    is overloaded it won't respond to tpmcoins quickly and the remote node can
            //    avoid sending us more work, like chain download requests.
            //
            // The nonce stops the remote getting confused between different tpmcoins: without
            // it, if the remote node sends a tpmcoin once per second and this node takes 5
            // seconds to respond to each, the 5th tpmcoin the remote sends would appear to
            // return very quickly.
    ...
    {
        int64_t tpmcoinUsecEnd = nTimeReceived;
        uint64_t nonce = 0;
        size_t nAvail = vRecv.in_avail();
        bool bTpmCoinFinished = false;
        std::string sProblem;

        if (nAvail >= sizeof(nonce)) {
            vRecv >> nonce;

            // Only process pong message if there is an outstanding tpmcoin (old tpmcoin without nonce should never pong)
            if (pfrom->nTpmCoinNonceSent != 0) {
                if (nonce == pfrom->nTpmCoinNonceSent) {
                    // Matching pong received, this tpmcoin is no longer outstanding
                    bTpmCoinFinished = true;
                    int64_t tpmcoinUsecTime = tpmcoinUsecEnd - pfrom->nTpmCoinUsecStart;
                    if (tpmcoinUsecTime > 0) {
                        // Successful tpmcoin time measurement, replace previous
                        pfrom->nTpmCoinUsecTime = tpmcoinUsecTime;
                    } else {
                        // This should never happen
                        sProblem = "Timing mishap";
                    }
                } else {
                    // Nonce mismatches are normal when tpmcoins are overlaptpmcoin
                    sProblem = "Nonce mismatch";
                    if (nonce == 0) {
                        // This is most likely a bug in another implementation somewhere, cancel this tpmcoin
                        bTpmCoinFinished = true;
                        sProblem = "Nonce zero";
                    }
                }
            } else {
                sProblem = "Unsolicited pong without tpmcoin";
            }
        } else {
            // This is most likely a bug in another implementation somewhere, cancel this tpmcoin
            bTpmCoinFinished = true;
        ...
        if (!(sProblem.empty())) {
            printf("pong %s %s: %s, %"PRIx64" expected, %"PRIx64" received, %zu bytes\n"
                ...
                , sProblem.c_str()
                , pfrom->nTpmCoinNonceSent
                , nonce
                , nAvail);
        }
        if (bTpmCoinFinished) {
            pfrom->nTpmCoinNonceSent = 0;
        }
    }
    ...
    if (pfrom->fNetworkNode)
        if (strCommand == "version" || strCommand == "addr" || strCommand == "inv" || strCommand == "getdata" || strCommand == "tpmcoin")
...
bool SendMessages(CNode* pto, bool fSendTrickle)
{
    ...
    if (lockMain) {
        ...
        // Message: tpmcoin
        //
        bool tpmcoinSend = false;
        if (pto->fTpmCoinQueued) {
            // RPC tpmcoin request by user
            tpmcoinSend = true;
        }
        if (pto->nTpmCoinNonceSent == 0 && pto->nTpmCoinUsecStart + TPM_INTERVAL * 1000000 < GetTimeMicros()) {
            // TpmCoin automatically sent as a latency probe & keepalive.
            tpmcoinSend = true;
        }
        if (tpmcoinSend) {
            uint64_t nonce = 0;
            ...
            pto->fTpmCoinQueued = false;
            pto->nTpmCoinUsecStart = GetTimeMicros();
            if (pto->nVersion > BIP0031_VERSION) {
                pto->nTpmCoinNonceSent = nonce;
                pto->PushMessage("tpmcoin", nonce);
            } else {
                // Peer is too old to support tpmcoin command with nonce, pong will never arrive.
                pto->nTpmCoinNonceSent = 0;
                pto->PushMessage("tpmcoin");
            }
        }
