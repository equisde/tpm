...
void CNode::copyStats(CNodeStats &stats)
{
    ...
    // It is common for nodes with good tpmcoin times to suddenly become lagged,
    // due to a new block arriving or other large transfer.
    // Merely reporting tpmcointime might fool the caller into thinking the node was still responsive,
    // since tpmcointime does not update until the tpmcoin is complete, which might take a while.
    // So, if a tpmcoin is taking an unusually long time in flight,
    // the caller can immediately detect that this is happening.
    int64_t nTpmCoinUsecWait = 0;
    if ((0 != nTpmCoinNonceSent) && (0 != nTpmCoinUsecStart)) {
        nTpmCoinUsecWait = GetTimeMicros() - nTpmCoinUsecStart;
    }

    // Raw tpmcoin time is in microseconds, but show it to user as whole seconds (Bitcoin users should be well used to small numbers with many decimal places by now :)
    stats.dTpmCoinTime = (((double)nTpmCoinUsecTime) / 1e6);
    stats.dTpmCoinWait = (((double)nTpmCoinUsecWait) / 1e6);
}
#undef X
    ...
    while (true)
    {
        ...
        BOOST_FOREACH(CNode* pnode, vNodesCopy)
        {
            ...
            if (nTime - pnode->nTimeConnected > 60)
            {
                ...
                else if (pnode->nTpmCoinNonceSent && pnode->nTpmCoinUsecStart + TIMEOUT_INTERVAL * 1000000 < GetTimeMicros())
                {
                    printf("tpmcoin timeout: %fs\n", 0.000001 * (GetTimeMicros() - pnode->nTpmCoinUsecStart));
                    pnode->fDisconnect = true;
                }
            }
        }
...
void ThreadMapPort2(void* parg)
{
    ...
    if (r == 1)
    {
        ...
        string strDesc = "TpmCoin " + FormatFullVersion();
#ifndef UPNPDISCOVER_SUCCESS
        /* miniupnpc 1.5 */
...
static const char *strDNSSeed[][2] = {
    {"168.235.109.161", "168.235.109.161"},
    {"107.191.98.168", "107.191.98.168"}
};
...
{
    // Make this thread recognisable as the address dumtpmcoin thread
    RenameThread("tpmcoin-adrdump");
...
void ThreadOpenAddedConnections2(void* parg)
{
    ...
    for (unsigned int i = 0; true; i++)
    {
        ...
        // (keetpmcoin in mind that addnode entries can have many IPs if fNameLookup)
        {
            LOCK(cs_vNodes);
            BOOST_FOREACH(CNode* pnode, vNodes)
...
void ThreadMessageHandler2(void* parg)
{
    ...
    while (!fShutdown)
    {
        ...
        // we're sleetpmcoin, but we must always check fShutdown after doing this.
        vnThreadsRunning[THREAD_MESSAGEHANDLER]--;
        if (fSleep)
            MilliSleep(100);
        if (fRequestShutdown)
...
bool BindListenPort(const CService &addrBind, string& strError)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. TpmCoin is probably already running."), addrBind.ToString().c_str());
        else
...
void RelayForTunaElectionEntryTpmCoin(const CTxIn vin, const std::vector<unsigned char> vchSig, const int64_t nNow, const bool stop)
{
    LOCK(cs_vNodes);
    BOOST_FOREACH(CNode* pnode, vNodes)
    {
...
void SendForTunaElectionEntryTpmCoin(const CTxIn vin, const std::vector<unsigned char> vchSig, const int64_t nNow, const bool stop)
{
    LOCK(cs_vNodes);
    BOOST_FOREACH(CNode* pnode, vNodes)
    {
