...
/** Time between tpmcoins automatically sent out for latency probing and keepalive (in seconds). */
static const int TPM_INTERVAL = 2 * 60;
/** Time after which to disconnect, after waiting for a tpmcoin response (or inactivity). */
...
public:
    ...
    // TpmCoin time measurement:
    // The pong reply we're expecting, or 0 if no pong expected.
    uint64_t nTpmCoinNonceSent;
    // Time (in usec) the last tpmcoin was sent, or 0 if no tpmcoin was ever sent.
    int64_t nTpmCoinUsecStart;
    // Last measured round-trip time.
    int64_t nTpmCoinUsecTime;
    // Whether a tpmcoin is requested.
    bool fTpmCoinQueued;
...
public:
    ...
    void EndMessage() UNLOCK_FUNCTION(cs_vSend)
    {
        ...
        {
            printf("dropmessages DROPTPM SEND MESSAGE\n");
            AbortMessage();
            return;
        }
...
template<typename T1, typename T2, typename T3, typename T4, typename T5, typename T6, typename T7, typename T8, typename T9, typename T10>
    ...
    // Droptpmcoin a node for sending stuff that is invalid
