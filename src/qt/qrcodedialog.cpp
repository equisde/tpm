    ...
    address(addr)
{
    ...
    if (label.trimmed().isEmpty()) {
        setWindowTitle(QString("TpmCoin - Generate QR Code"));
    } else {
        setWindowTitle(QString("TpmCoin - Generate QR Code for %1").arg(label));
    }
...
{
    QString ret = QString("tpmcoin:%1").arg(address);
    int paramCount = 0;
...
{
    QString fn = GUIUtil::getSaveFileName(this, tr("Save QR Code"), QString(), tr("TPM Images (*.png)"));
    if (!fn.isEmpty())
