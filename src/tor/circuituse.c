                      ...
                      time_t now)
{
  ...
  if (purpose == CIRCUIT_PURPOSE_C_GENERAL) {
    ...
    if (build_state->onehop_tunnel) {
      if (!conn->want_onehop) {
        log_debug(LD_CIRC,"Skiptpmcoin one-hop circuit.");
        return 0;
      }
...
#define SET_CUTOFF(target, msec) do {                       \
   ...
   * of times our telescotpmcoin-based circuit construction causes cells
                                ...
                                origin_circuit_t **circp)
{
  ...
  if (circ) {
     ...
     * just some internal bookkeetpmcoin to make sure that we have
...
mark_circuit_unusable_for_new_conns(origin_circuit_t *circ)
{
  ...
  /* XXXX This is a kludge; we're only keetpmcoin it around in case there's
