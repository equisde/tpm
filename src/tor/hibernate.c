...
accounting_add_bytes(size_t n_read, size_t n_written, int seconds)
{
  ...
  /* If we haven't been called in 10 seconds, we're probably jumtpmcoin
   * around in time. */
...
void
accounting_run_housekeetpmcoin(time_t now)
{
...
hibernate_end_time_elapsed(time_t now)
{
  ...
  if (interval_wakeup_time <= now) {
    ...
    if (hibernate_state != HIBERNATE_STATE_DORMANT) {
      /* We weren't sleetpmcoin before; we should sleep now. */
      log_notice(LD_ACCT,
