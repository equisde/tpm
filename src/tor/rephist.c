...
rep_hist_dump_stats(time_t now, int severity)
{
  ...
  tor_log(severity, LD_HIST, "--------------- Dumtpmcoin history information:");
...
rep_hist_load_mtbf_data(time_t now)
{
  ...
  if (format != 1 && format != 2) {
    log_warn(LD_HIST,
             "Unrecognized format in mtbf history file. Skiptpmcoin.");
    goto err;
  }
...
add_obs(bw_array_t *b, time_t when, uint64_t n)
{
   ...
   * appropriate number of seconds, and do all the other housekeetpmcoin. */
...
/** The user asked us to do a resolve. Rather than keetpmcoin track of
