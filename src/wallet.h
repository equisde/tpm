...
enum AvailableCoinsType
{
    ...
    ONLY_NONDENOMINATED_NOTMN = 4 // ONLY_NONDENOMINATED and not 5000 TPM at the same time
};
...
public:
    ...
    std::set< std::set<CTxDestination> > GetAddressGroutpmcoins();
    ...
    bool GetCredit(const CTransaction& tx, int64_t& nTPM, int64_t& nAnon, const isminefilter& filter) const
    {
        nTPM = 0;
        nAnon = 0;
        BOOST_FOREACH(const CTxOut& txout, tx.vout)
...
public:
    ...
    mutable int64_t nCredTPMCached;
    ...
    bool GetCredit(int64_t& nCredTPM, int64_t& nCredAnon, bool fUseCache=true) const
    {
        // Must wait until coinbase is safely deep enough in the chain before valuing it
        if ((IsCoinBase() || IsCoinStake()) && GetBlocksToMaturity() > 0)
        {
            nCredTPM = nCredAnon = 0;
            return true;
        }
        ...
        if (!fUseCache || !fCreditSplitCached)
        {
            nCredTPMCached = 0;
            nCredAnonCached = 0;
            pwallet->GetCredit(*this, nCredTPMCached, nCredAnonCached, ISMINE_SPENDABLE);
            fCreditSplitCached = true;
        };

        nCredTPM = nCredTPMCached;
